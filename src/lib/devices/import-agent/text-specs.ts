...
export const TEXT_SPECS_FIELD_DICTIONARY = [
  ...
  'DISPLAY: size_inches (e.g. "6.78 inches" or 6.78), display_type (e.g. "AMOLED", "LTPO AMOLED"), resolution as ONE string like "2772x1272" in EITHER resolution_width or resolution_height (the normalizer splits WxH), refresh_hz (e.g. "120Hz" or "1-120Hz Adaptive" — put the whole phrase in, max number wins), adaptive_refresh ("ltpo" if LTPO/adaptive/1-120Hz is stated), peak_brightness_nits (e.g. "1800 nits" — use the HBM/peak figure), hdr (e.g. "HDR10+, HDR Vivid" — the normalizer maps it).',
  ...
  'CAMERA: an object with rear (array, one entry per lens: Main, Telephoto, Ultra-wide, Macro) and selfie (array). Each lens: type, megapixels (e.g. 50), sensor_model (e.g. "IMX906"), aperture as the f-string (e.g. "ƒ/1.8" or "f/2.8" — the normalizer parses it), ois ("yes" if Optical Image Stabilization is stated), af ("yes" or the AF type), focal_length_mm (e.g. "21 mm equivalent"), optical_zoom_x (e.g. "3.5X optical zoom"), video_modes (array of strings like "8K 30fps"). NEVER nest under "Main Camera"/"Telephoto Camera" headings — flatten into the rear array.',
